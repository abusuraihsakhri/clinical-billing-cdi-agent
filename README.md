...
**Clinical Billing Cdi Agent** is an advanced analytical and computational platform implementing Inpatient Documentation Gap & CMS-HCC Risk Adjustment Agent.
...
- **`DocumentationGapScannerAgent`**: Specialized Sub-Agent 1 for clinical-billing-cdi-agent
- **`HCCRiskWeightCalculatorAgent`**: Specialized Sub-Agent 2 for clinical-billing-cdi-agent
- **`PhysicianQuerySynthesizerAgent`**: Specialized Sub-Agent 3 for clinical-billing-cdi-agent
...
```bash
docker build -t clinical-billing-cdi-agent .
docker run -p 8000:8000 clinical-billing-cdi-agent
```
