import pytest
from cdi_sentinel import DocumentationGapScannerAgent, HCCRiskWeightCalculatorAgent, PhysicianQuerySynthesizerAgent, CDICoordinator, main


def test_sub_agents():
    a1 = DocumentationGapScannerAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = HCCRiskWeightCalculatorAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = PhysicianQuerySynthesizerAgent()
...
def test_domain_registry():
    from cdi_sentinel import DomainKnowledgeRegistry
